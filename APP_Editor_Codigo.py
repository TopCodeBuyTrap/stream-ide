...
completar = [# Palavras/chaves que aparecem no autocomplete (além das do idioma):
...
def editor_codigo_autosave(st, aba_id, caminho_arquivo, conteudo_inicial, linguagem, thema_editor, font_size):
    _ = st.session_state
    ...
    opcoes_editor = {
        ...
        "fontFamily": 'Consolas, "Courier New", monospace',  # ← ISSO RESOLVE  # Fonte (use 'Fira Code', 'JetBrains Mono', etc.)
    ...
    cod = code_editor(conteudo_inicial,
        ...
        key=f"code_editor_{aba_id}" # mudei o key pra não conflitar com ace
    )

    # Extrai código editado
    novo_codigo = cod.get('text', '') if isinstance(cod, dict) else str(cod) if cod else conteudo_atual

    # Auto-save (só se mudou) - SEM DELAY
    agora = time.time()
    ultimo_salvo = _.get(f"last_save_{aba_id}", "")

    if (novo_codigo.strip() and
        novo_codigo != ultimo_salvo and
        caminho_arquivo):

        _[content_key] = novo_codigo
        _[f"last_save_{aba_id}"] = novo_codigo
        _[f"save_ts_{aba_id}"] = agora
