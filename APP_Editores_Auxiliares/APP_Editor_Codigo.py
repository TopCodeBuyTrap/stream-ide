...
from APP_Code_Editor.Configurations import Opcoes_editor, Component_props, Atalhos, editor_props, Snippets
...
from Banco_dados.autosave_manager import get_max_version, salvar_versao
...
def editor_codigo_autosave_(st, aba_id,nome_arq, diretorio_arquivo,  linguagem, thema_editor, font_size, fonte, altura,Info_Col, backgroud):
    _ = st.session_state
    ...
    def renderizar_editor(conteudo_inicial, falta_modulo=None,
        ...
        try:
            ...
            props = {"markers": [], "annotations": [],"style": {"background": 'red'}}
        ...
        _LOGS_popover(Info_Col, f":material/code: Logs de Renderização e Configuração do Editor", editor_logs)
    ...
    codigo, conteudo_key = renderizar_editor(conteudo_inicial, falta_modulo=_['falta_modulo'],botoes_definicao=botoes_definicao)
...
def editor_codigo_autosave__(st, aba_id,nome_arq, diretorio_arquivo,  linguagem, thema_editor, font_size, fonte, altura,Info_Col, backgroud):
    _ = st.session_state
...
def editor_codigo_autosave(st, aba_id, nome_arq, diretorio_arquivo, linguagem, thema_editor, font_size, fonte, altura, Info_Col, backgroud):
    ...
    # ========================================
    # PRIMEIRO BLOCO: INICIALIZAÇÃO DE VERSÃO (MELHORADO)
    # ========================================
    version_key = f'version_{aba_id}_{nome_arq}'
    if version_key not in _:
        max_v = get_max_version(diretorio_arquivo)
        _[version_key] = max_v if max_v > 0 else 1
        if max_v == 0:
            salvar_versao(diretorio_arquivo, conteudo_inicial)
            _[version_key] = 1
    # ========================================
    ...
    def renderizar_editor(conteudo_inicial, falta_modulo=None,
        ...
        # ========================================
        # SALVAMENTO DE VERSÃO A CADA BLUR (NOVO)
        # ========================================
        if codigo and 'text' in codigo:
            novo_conteudo_blur = codigo['text']
            if novo_conteudo_blur != _[conteudo_key]:  # Só salva se mudou
                new_version_id = salvar_versao(diretorio_arquivo, novo_conteudo_blur)
                _[version_key] = new_version_id
                _[conteudo_key] = novo_conteudo_blur  # Atualiza o conteúdo
        # ========================================
    ...
    # ← CORREÇÃO: Retorna o TEXTO do código (string), não o dicionário inteiro
    return codigo['text'] if codigo and 'text' in codigo else ""